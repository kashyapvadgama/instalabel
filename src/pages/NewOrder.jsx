...
export default function NewOrder({ session, setView }) {
  ...
  // 🟢 FETCH STORE PROFILE ON MOUNT (Fix for H-01)
  useEffect(() => {
  ...
  // 🟢 CRITICAL FIX (C-01): Upload ALL files
  const handleSaveAndPrint = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('orders').insert([{ 
          ...
          screenshot_url: uploadedPaths, // Saving Array
          ...formData, 
          status: 'pending' 
      }]);
      ...
      // Pass Store Profile to PDF Generator
  ...
  return (
    // ... (UI Code remains exactly the same as before, no changes needed in Return block)
          ...
          <div className="col-span-12 md:col-span-4 lg:col-span-3 bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
            ...
            <div className="overflow-y-auto flex-1 divide-y divide-gray-100">
              {queue.map((item, index) => (
                <div key={item.id} onClick={() => setSelectedIndex(index)} className={`p-3 flex items-start gap-3 cursor-pointer transition-colors ${selectedIndex === index ? 'bg-blue-50' : 'hover:bg-gray-50'}`}>
                  <button onClick={(e) => { e.stopPropagation(); toggleSelection(item.id); }} className="mt-1 text-gray-400 hover:text-blue-600" aria-label={selectedIds.includes(item.id) ? "Deselect item" : "Select item for merge"}>
